package com.openclaw.service.auth.impl;
...
public class AuthServiceImpl extends ServiceImpl<UserAccountMapper, UserAccount> implements AuthService {
    ...
    public AuthServiceImpl(@Autowired(required = false) StringRedisTemplate redisTemplate,
                           ...
                           @Value("${openclaw.auth.redis-enabled:true}") boolean redisEnabled,
                           @Value("${openclaw.auth.default-role:USER}") String defaultRole,
                           @Value("${openclaw.auth.anonymous-role:USER}") String anonymousRole,
                           @Value("${openclaw.auth.bootstrap-first-user-admin:true}") boolean bootstrapFirstUserAdmin,
                           @Value("${openclaw.auth.token-ttl-hours:24}") int tokenTtlHours,
                           @Value("${openclaw.auth.password-pepper:}") String passwordPepper,
                           @Value("${openclaw.auth.token-prefix:openclaw:auth:token:}") String tokenPrefix) {
        ...
        this.tokenPrefix = StringUtils.hasText(tokenPrefix) ? tokenPrefix.trim() : "openclaw:auth:token:";
    }
