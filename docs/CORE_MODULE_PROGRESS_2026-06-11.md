...
| Self Evolution | `AgentLearningService`、`AgentRunTraceService`、`docs/memory-bank/agent-learnings.md` | 从失败 trace 中提炼 lesson/rule/counterexample/evidence，去重后写入可审阅 Memory Bank，并在下一轮上下文优先加载 | 2 | 35% | 下一步增加用户确认、反例分类、规则失效回滚；不要让模型自动无限写规则 |
...
- 条目包含 `requestId/source/trigger/lesson/rule/counterexample/evidence/signature`，用于表达“从执行中学到的规则和反例”。
...
- `MemoryBankService` 已优先读取 `agent-learnings.md`，让经验能进入下一轮 Context。
...
- 为学习条目增加用户确认/撤销机制，避免错误规则污染上下文。
- 把 Obsidian / Wiki.js 作为 Markdown Knowledge Source 接入，不把它们直接混进用户长期记忆或向量 RAG。
- 增加 learning hit count：统计哪些 learning 被拼入上下文、是否影响后续成功率。
