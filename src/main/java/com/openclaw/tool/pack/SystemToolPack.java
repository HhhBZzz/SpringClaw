...
public class SystemToolPack {
    ...
    @Autowired
    public SystemToolPack(@Value("${openclaw.tools.system.command-enabled:false}") boolean commandEnabled,
                          @Value("${openclaw.tools.system.command-mode:whitelist}") String commandMode,
                          @Value("${openclaw.tools.system.allowed-commands:pwd,ls,date,whoami,uname,id}") String allowedCommands,
                          @Value("${openclaw.tools.system.blocked-commands:rm,dd,shutdown,reboot,halt,poweroff,mkfs,fdisk,kill,killall,pkill}") String blockedCommands,
                          @Value("${openclaw.tools.system.command-timeout-seconds:5}") int commandTimeoutSeconds,
                          @Value("${openclaw.tools.system.max-output-chars:2000}") int maxOutputChars) {
    ...
    public String runCommand(String commandLine) {
        if (!commandEnabled) {
            return "命令执行能力未开启（openclaw.tools.system.command-enabled=false）";
        }
