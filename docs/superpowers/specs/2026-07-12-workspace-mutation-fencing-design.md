...
获取租约和整个工具执行位于同一个数据库事务：先 `INSERT IGNORE` 初始化行，再 `SELECT ... FOR UPDATE`，并把行锁一直持有到工具执行、Git 发布和 proposal 成功终态全部完成。只有无持有者或 `lease_until <= CURRENT_TIMESTAMP(6)` 时才能把 token 加一并设置新持有者。竞争执行者会在数据库行锁上等待或按数据库锁等待超时失败，不能在旧执行者仍存活时接管共享工作区。
...
- MySQL 集成测试验证互斥获取、过期接管时 token 递增、旧 token 不能校验或释放新租约，并用两个独立事务证明完整执行期间竞争者不能进入。
