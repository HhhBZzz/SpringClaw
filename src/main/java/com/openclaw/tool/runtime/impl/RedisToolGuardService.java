package com.openclaw.tool.runtime.impl;

import com.openclaw.common.exception.BusinessException;
import com.openclaw.tool.runtime.ToolGuardService;
...
public class RedisToolGuardService implements ToolGuardService {
    ...
    public RedisToolGuardService(@Autowired(required = false) StringRedisTemplate redisTemplate,
                                 @Value("${openclaw.tools.guard.enabled:true}") boolean enabled,
                                 @Value("${openclaw.tools.guard.redis-enabled:true}") boolean redisEnabled,
                                 @Value("${openclaw.tools.guard.max-calls:60}") int maxCalls,
                                 @Value("${openclaw.tools.guard.window-seconds:60}") int windowSeconds,
                                 @Value("${openclaw.tools.guard.redis-retry-seconds:30}") int redisRetrySeconds) {
    ...
    public void checkRateLimit(String toolName) {
        ...
        if (canUseRedis()) {
            try {
                String redisKey = "openclaw:tool:rate:" + key;
