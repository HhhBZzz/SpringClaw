...
| Knowledge Source / Wiki | `MarkdownKnowledgeSourceService`、`RuntimeKnowledgeSourceController`、Runtime Console Knowledge 面板、`docs/memory-bank`、未来 Obsidian/Wiki.js Markdown source | 已有只读 Markdown Knowledge Source 底座，可扫描 Wiki.js / Obsidian 风格导出的 Markdown；只有 front matter `status: active/approved` 的文档会进入 snapshot，无 status/draft/rejected 等会被过滤；后端已提供 ADMIN review/list 与 snapshot preview 接口，前端已可展示项目知识来源、context impact 和 `injectedToRuntimePrompt=false`；当前尚未接 Wiki API，也未接入 RAG 或用户长期记忆 | 3 | 40% | 下一步保持 review-only，先补人工状态治理和来源审计；不要现在接复杂 Wiki API 或向量 RAG 管道 |
...
- 条目包含 `status/requestId/source/trigger/lesson/rule/counterexample/evidence/signature`，用于表达“从执行中学到的规则和反例”；review DTO 会派生 `counterexampleCategory/contextIncluded/contextImpact`，不写回 Markdown schema。
...
- `AgentLearningService.listEntries` 已支持按 limit 读取 review 条目，返回 signature、status、trigger、lesson、rule、counterexample、counterexampleCategory、contextIncluded、contextImpact、evidence、reviewedAt/reviewReason 等字段。
...
- Agent Console 的 Runtime Resource 面板已增加 `Learning` 入口，可展示 rule/lesson/counterexample/counterexample type/context impact/evidence，填写 review reason，并对规则执行 active/approved/disabled/rejected/superseded。

验证：

- `mvn -Dtest=AgentLearningServiceTest,RuntimeLearningControllerTest test`
...
- 继续把 Obsidian / Wiki.js 作为受控 Markdown Knowledge Source：当前已有只读 review/list 与 snapshot preview，下一步再决定是否提供人工状态治理或受控上下文注入点。
- 继续补 learning 影响复盘：把 active learning count、具体 rule signature 和后续任务成功/失败 trace 关联起来。

### Step 4.4：Markdown Knowledge Source 只读底座已落地

当前状态：

- `MarkdownKnowledgeSourceService` 已提供只读 Markdown snapshot，面向 Wiki.js / Obsidian 导出的项目知识。
- `RuntimeKnowledgeSourceController` 已提供 ADMIN 只读 review/list 与 snapshot preview 接口。
- Runtime Console 已提供 Knowledge 面板展示来源状态、context impact、snapshot 字符数和 `injectedToRuntimePrompt=false`。
- Knowledge Source 与用户 Long-term Memory 分离；当前不写入 `VectorMemoryService`，不写入 `agent-learnings.md`，也不接入 RAG。
- 只有 Markdown front matter 中 `status: active` 或 `status: approved` 的文档会进入 snapshot；无 status、draft、rejected 等文档会被过滤，避免默认污染上下文。
- snapshot 会输出 `includedCount/filteredCount/context`，并以 `### knowledge-source/<relative-path>` 标注来源，front matter 不进入 prompt 文本。
- 当前服务尚未接入主 Runtime prompt；这是给后续人工治理或受控上下文注入点准备的基础设施。

验证：

- `mvn -Dtest=MarkdownKnowledgeSourceServiceTest test`
