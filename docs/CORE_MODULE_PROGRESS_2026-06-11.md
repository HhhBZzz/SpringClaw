...
| Knowledge Source / Wiki | `MarkdownKnowledgeSourceService`、`docs/memory-bank`、未来 Obsidian/Wiki.js Markdown source | 已有只读 Markdown Knowledge Source 底座，可扫描 Wiki.js / Obsidian 风格导出的 Markdown；只有 front matter `status: active/approved` 的文档会进入 snapshot，无 status/draft/rejected 等会被过滤；当前尚未接 Wiki API，也未接入 RAG 或用户长期记忆 | 2 | 28% | 下一步先暴露 review/list 能力或受控注入点；不要现在接复杂 Wiki API 或向量 RAG 管道 |
...
- 继续把 Obsidian / Wiki.js 作为受控 Markdown Knowledge Source：当前已有只读 snapshot 底座，下一步再决定是否展示 review/list 或进入受控上下文注入点。
...
- 当前服务尚未接入主 Runtime prompt；这是给后续 review/list UI 或受控上下文注入点准备的基础设施。
