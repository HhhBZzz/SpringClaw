...
| Knowledge Source / Wiki | `MarkdownKnowledgeSourceService`、`RuntimeKnowledgeSourceController`、Runtime Console Knowledge 面板、`MessageEventService`、`docs/memory-bank`、未来 Obsidian/Wiki.js Markdown source | 已有 Markdown Knowledge Source 治理底座，可扫描 Wiki.js / Obsidian 风格导出的 Markdown；只有 front matter `status: active/approved` 的文档会进入 snapshot，无 status/draft/disabled/rejected 等会被过滤；后端已提供 ADMIN review/list、snapshot preview 与 status 更新接口，前端已可展示项目知识来源、context impact、reviewedAt/reviewReason，填写 review reason 并执行 active/approved/disabled/rejected；status 更新已写入 `MessageEventService` 结构化审计事件，schema 为 `springclaw.knowledge-source-review.v1`；当前仍显示 `injectedToRuntimePrompt=false`，尚未接 Wiki API，也未接入 RAG 或用户长期记忆 | 3 | 54% | 下一步再评估是否把知识源审核事件镜像到 run trace 或前端审计查询；不要现在接复杂 Wiki API、向量 RAG 管道或自动注入主 prompt |
...
- 继续把 Obsidian / Wiki.js 作为受控 Markdown Knowledge Source：当前已有 review/list、snapshot preview、人工 status 治理、可见 reviewedAt/reviewReason，并已把 status 变更写入 MessageEvent 审计流；下一步再决定是否镜像到 run trace 或提供受控上下文注入点。
...
- status 更新成功后，`RuntimeKnowledgeSourceController` 会写入 `MessageEventService`，事件类型为 `KNOWLEDGE_SOURCE`，content 为 `springclaw.knowledge-source-review.v1` JSON，用于后续审计查询或 trace 镜像。
...
- 当前服务尚未接入主 Runtime prompt；这是给后续来源审计或受控上下文注入点准备的基础设施。

验证：

- `mvn -Dtest=MarkdownKnowledgeSourceServiceTest,RuntimeKnowledgeSourceControllerTest test`
