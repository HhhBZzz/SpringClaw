...
public class ExchangeRateToolPack {
    ...
    @Autowired
    public ExchangeRateToolPack(@Value("${openclaw.tools.exchange.enabled:true}") boolean enabled,
                                @Value("${openclaw.tools.exchange.url-template:https://open.er-api.com/v6/latest/{base}}") String urlTemplate,
                                @Value("${openclaw.tools.exchange.timeout-seconds:8}") int timeoutSeconds,
