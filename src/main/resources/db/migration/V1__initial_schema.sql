...
CREATE TABLE IF NOT EXISTS `message_event` (
  ...
  UNIQUE KEY `uk_message_event_event_key` (`event_key`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS `memory_record` (
  ...
  UNIQUE KEY `uk_memory_source_policy` (`source_kind`, `source_identity`, `extraction_policy_version`, `memory_type`),
  KEY `idx_memory_scope_active` (`scope_type`, `scope_id`, `status`, `deleted`, `update_time`),
...
CREATE TABLE IF NOT EXISTS `memory_index_outbox` (
  ...
  UNIQUE KEY `uk_memory_outbox_revision` (`logical_memory_id`, `index_revision`, `operation`),
  KEY `idx_memory_outbox_claim` (`status`, `available_at`, `lease_until`, `id`),
  KEY `idx_memory_outbox_logical_revision` (`logical_memory_id`, `index_revision`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
