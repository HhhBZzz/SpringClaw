...
- Knowledge Source 方向已新增只读 Markdown review/list + snapshot preview 底座：Wiki.js / Obsidian 导出的 Markdown 只有 front matter `status: active/approved` 才会进入 project knowledge snapshot；Runtime Console 可展示来源状态、context impact 和 `injectedToRuntimePrompt=false`；它尚未接入 RAG，也不写入用户长期记忆。
...
3. 让 Obsidian / Wiki.js 这类 Markdown 知识源继续走受控 Knowledge Source review/list、snapshot preview 或上下文注入点，而不是污染用户长期记忆。
