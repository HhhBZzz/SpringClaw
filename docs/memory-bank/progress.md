...
- Runtime Console 已提供 ADMIN learning review 入口：可列表化规则、按状态筛选、填写 review reason，并在前端执行 active/approved/disabled/rejected/superseded。

当前正在推进：

- Context 可解释与低基数观测。
- 自进化经验沉淀、人工审核、状态筛选、状态修正和后续影响复盘，而不是只保存对话。
