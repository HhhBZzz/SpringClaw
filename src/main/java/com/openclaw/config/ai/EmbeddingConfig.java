 ...
 */
@Configuration
@ConditionalOnProperty(prefix = "openclaw.embedding", name = "enabled", havingValue = "true")
public class EmbeddingConfig {
    ...
    public EmbeddingModel openClawEmbeddingModel(OpenClawEmbeddingProperties properties,
                                                 ObjectProvider<RestClient.Builder> restClientBuilderProvider) {
        if (!StringUtils.hasText(properties.getApiKey())) {
            throw new IllegalStateException("Embedding 已启用，但未配置 OPENCLAW_EMBEDDING_API_KEY");
        }
    ...
    public VectorStore openClawRedisVectorStore(EmbeddingModel embeddingModel,
                                                ...
                                                @org.springframework.beans.factory.annotation.Value("${spring.ai.vectorstore.redis.index-name:openclaw-memory}") String indexName,
                                                @org.springframework.beans.factory.annotation.Value("${spring.ai.vectorstore.redis.prefix:openclaw:}") String prefix,
