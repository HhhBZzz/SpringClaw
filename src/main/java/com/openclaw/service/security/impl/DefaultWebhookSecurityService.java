package com.openclaw.service.security.impl;

import com.openclaw.common.exception.BusinessException;
import com.openclaw.service.security.WebhookSecurityService;
...
public class DefaultWebhookSecurityService implements WebhookSecurityService {
    ...
    public DefaultWebhookSecurityService(@Autowired(required = false) StringRedisTemplate redisTemplate,
                                         @Value("${openclaw.webhook.security.enabled:false}") boolean enabled,
                                         @Value("${openclaw.webhook.security.redis-enabled:true}") boolean redisEnabled,
                                         @Value("${openclaw.webhook.security.replay-seconds:300}") long replaySeconds,
                                         @Value("${openclaw.webhook.security.default-secret:}") String defaultSecret,
                                         @Value("${openclaw.webhook.security.channel-secrets.telegram:}") String telegramSecret,
                                         @Value("${openclaw.webhook.security.channel-secrets.wechat:}") String wechatSecret,
                                         @Value("${openclaw.webhook.security.channel-secrets.feishu:}") String feishuSecret) {
    ...
    public void verify(String channel, Map<String, String> headers, String rawBody) {
        ...
        String timestamp = header(headers, "x-openclaw-timestamp");
        String nonce = header(headers, "x-openclaw-nonce");
        String signature = header(headers, "x-openclaw-signature");
    ...
    private void ensureNotReplayed(String channel, long ts, String nonce) {
        String nonceKey = "openclaw:webhook:nonce:" + normalize(channel) + ":" + ts + ":" + nonce;
